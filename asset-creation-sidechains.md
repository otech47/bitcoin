...
For all intents and purposes of this example, mining a new block on the sidechain and broadcasting a new state of the payment channel are analogous events. Since it is a new sidechain, the proof required to add a new block is arbitrary and can be reduced to a new channel state signed with both keys (a sort of "proof-of-stake" consensus between 2 nodes, depending on your flavor of buzzwords). No additional energy requirement or special hashing is necessary to secure the only transaction currently present in the genesis block of the sidechain: a single coinbase transaction to an address holding 100 million satoshis.
